...
<nav class="page-sequence"><a href="cours/chapitre-5/2-pourquoi-automatiser-dans-le-cloud">Pr&eacute;c&eacute;dent</a> <a href="cours/chapitre-5/index">Sommaire</a> <a href="cours/chapitre-5/4-aws-systems-manager-automatisation-operationnelle">Suivant</a></nav>
...
Cette étape 3 (« AWS provisionne dans le bon ordre ») repose sur une propriété centrale de CloudFormation : l'**idempotence**. Relancer un déploiement avec un template inchangé ne doit rien recréer, puisque l'état réel du compte correspond déjà à l'état déclaré dans le template — CloudFormation compare avant d'agir, il n'exécute pas bêtement la liste des ressources à chaque appel. C'est ce qui rend un déploiement CloudFormation reproductible sans effet de bord : l'exécuter une fois ou dix fois produit le même résultat final.
...
<strong>Réflexe d'automatisation — idempotence CloudFormation</strong>
